...
function font:saveImage(path, width, height)
	...
	local meta = "-- File generated by https://github.com/Dantevg/bitmapfontcreator\n"
	meta = meta .. string.format("return {\n\tfile=%q,\n\theight=%d,\n",
...
function font:autoCompoundGlyphs(layer)
	...
	local function autocombine( name )
		local base = { glyphs[name:sub(1,1)], 0, 0 }
		local accent = { glyphs[name:sub(2).."cmb"], 0, function() return base[1].height end }
		combine( name, base, accent )
	end
	...
	combine( "AE",
		{glyphs.A, 0, 0},
		{glyphs.E, function() return glyphs.A.width end, 0} )
	
	combine( "Ccedilla",
		{glyphs.C, 0, 0},
		{glyphs.cedillacmb, 0, function() return -glyphs.cedillacmb.height end} )
